...
class Grapher extends JPanel implements ActionListener {
	...
	public void initializePoints() {
		if (graph == true) {
			...
			for (int i = 0; i < STEPS + 1; i++) {
				...
				if (leminiscate){
					if (cCo*Math.cos((theta + ctR )* ctC) < 0 || sCo*Math.sin((theta + stR) * stC) < 0){
						radii[i] = 0;
					}
	...
	public void initializeOptionButtons() {
		...
		basicLeminiscate.addActionListener(new Leminiscate());
	}
	
	public void initializeLeminiscate(){
		add(lemk);
		lemk.setBounds(745, 20, 30, 30);
		lemk.setVisible(false);
		
		add(lemtrigC);
		lemtrigC.setVisible(false);
		lemtrigC.setBounds(800, 20 , 30, 30);
			
		add(lemR);
		lemR.setBounds(924, 20, 50, 30);
		lemR.setVisible(false);
		add(lemtC);
		lemtC.setBounds(863, 20, 30, 30);
		lemtC.setVisible(false);
	...
	public void initializeBliEquation() {
		...
		blitrigC.setVisible(false);
		sine.setBounds(740, 67, 70, 25);
		cosine.setBounds(820, 67, 70, 25);
		sine.setVisible(false);
		cosine.setVisible(false);
		add(sine);
		add(cosine);
	...
	public void paintComponent(Graphics g) {
		...
		if (leminiscate){
			g.setFont(new Font("Plain", Font.PLAIN, 10));
			g.drawString("2", 726, 28);
		}
		...
		else if (leminiscate) drawLeminiscate(g);
	...
	public void drawLeminiscate(Graphics g){
		g.drawString(" + ", 775, 40);
		if (cos)
			g.drawString("cos(", 830, 40);
		else
